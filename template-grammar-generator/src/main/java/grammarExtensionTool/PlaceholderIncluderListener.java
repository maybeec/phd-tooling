...
import antlr4Parser.ANTLRv4Parser.TerminalContext;
...
public class PlaceholderIncluderListener extends ANTLRv4ParserBaseListener {
    ...
    public void exitGrammarSpec(GrammarSpecContext ctx) {
        ...
        // insert Placeholder
        rewriter.insertAfter(ctx.stop, "\n" + uniquePlaceholderStart + placeHolderName + ": "
            + placeHolderDef + ";");
        ...
        // rename grammar
        rewriter.replace(ctx.id().stop, newGrammarName);
    }
        ...
        if (terminal.startsWith("'") && terminal.endsWith("'")) {
            ...
            while (currentParent != null) {
                if (currentParent instanceof ParserRuleSpecContext) {
                    ...
                    if (tokenName == null) {
                        System.out
                            .println("Did not find appropriate Tokenname for clean text in a parser rule: "
                                + terminal);
                        uniqueNameCounter++;
                        String newLexerRuleName =
                            uniquePlaceholderStart + "ImplicitToken" + uniqueNameCounter;
