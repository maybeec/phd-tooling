...
public abstract class LexerAdaptor extends Lexer {
     ...
     * _currentRuleType==Token.INVALID_TYPE means that we are outside of a rule. At the first sign of a rule
     * name reference and _currentRuleType==invalid, we can assume that we are starting a parser rule.
     * Similarly, seeing a token reference when not already in rule means starting a token rule. The
     * terminating ';' of a rule, flips this back to invalid type.
     *
     * This is not perfect logic but works. For example, "grammar T;" means that we start and stop a lexical
     * rule for the "T;". Dangerous but works.
     *
     * The whole point of this state information is to distinguish between [..arg actions..] and [charsets].
