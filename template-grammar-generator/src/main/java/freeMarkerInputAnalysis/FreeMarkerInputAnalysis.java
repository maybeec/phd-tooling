package freeMarkerInputAnalysis;
...
public class FreeMarkerInputAnalysis {
    ...
    /**
     * An internally used path to store temporary filey
     */
    private String templatePath = "src/main/java/freeMarkerInputAnalysis/";
     ...
     */
    public boolean isValidInput(Object inputData, List<Map<String, String>> placeholderTypeList,
                ...
                if (Character.isUpperCase(expectedRuleName.charAt(0))) {
                    ...
                    if (tokenList.size() != 2) {
                        ...
                        if (!actualTokenName.equals(expectedRuleName)) {
                            System.out.println(actualData + " is " + actualTokenName + " but expected "
                                + expectedRuleName);
                            isValid = false;
                            break;
                        }
                    ...
                    tokens.fill();
                    if (!(objectLangParser.getCurrentToken() == tokens.getTokens().get(
     ...
     */
    private String getActualValue(String placehoderName, Object inputData) throws IOException,
        TemplateException {

        // simulate template
        String tempName = "temp.ftl";
        FileWriter fileWriter = new FileWriter(templatePath + "/" + tempName);
        BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
        bufferedWriter.write(placehoderName);
        bufferedWriter.close();

        // evaluate simulated template
        cfg.clearTemplateCache();
        Template temp = cfg.getTemplate(tempName);
        File outputfile = new File(templatePath + "/" + "value.txt");
