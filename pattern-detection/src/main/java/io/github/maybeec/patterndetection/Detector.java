...
import io.github.maybeec.patterndetection.common.CommonParseTools;
...
public class Detector {
    ...
    public List<Map<String, String>> detect(Iterable<Path> pattern, Iterable<Path> applicationFiles,
        ...
        for (Path template : pattern) {
            templateParseTrees.put(template,
                CommonParseTools.parseAmbiguities(template.toFile(), PredictionMode.LL_EXACT_AMBIG_DETECTION, false));
        }
        ...
        for (Path appFile : applicationFiles) {
            applicationParseTrees.put(appFile, CommonParseTools.parse(appFile.toFile(), PredictionMode.LL, false));
        }
